...
class MapManager:
    ...
    def load_map(self, grid, filename=None):
        ...
        try:
            ...
            # Поддержка старого формата (filled_cells) и нового формата (cells)
            cells_data = data.get('filled_cells') or data.get('cells')
            
            if cells_data is None:
                print(f"Ошибка: не найден ключ 'cells' или 'filled_cells' в файле")
                return False
            ...
            print(f"Карта загружена: {filename} ({len(cells_data)} клеток)")
            return True
