...
class Renderer:
    ...
    def draw_sight_zone(self, sight, grid):
        ...
        Args:
            ...
            grid: объект Grid
        """
        # TODO: Рисуем все видимые клетки (Задание 4)
        for col, row in sight.visible_cells:  # sight.visible_cells
            x = col * grid.cell_width + grid.offset_x
            y = row * grid.cell_height

            # TODO: Определить цвет и прозрачность
            if (col, row) in sight.obstacle_cells:  # (col, row) in sight.obstacle_cells
                color = (100, 150, 255)  # (100, 150, 255) - светло-синий
                alpha = 150  # 150 - более непрозрачный
            else:
                color = (70, 70, 150)  # (70, 70, 150) - тусклый
                alpha = 100  # 30 - прозрачный
        ...
        # TODO: Рисуем центр масс если есть препятствия
        if not sight.center_of_mass is None:  # sight.center_of_mass
            cx, cy = sight.center_of_mass  # sight.center_of_mass
    ...
    def draw_sight_line(self, robot, sight):
        ...
        Args:
            ...
            sight: объект Sight
        """
        # TODO: Рисуем линию до центра масс (Задание 4)
        if not sight.center_of_mass is None:  # sight.center_of_mass
            cx, cy = sight.center_of_mass  # sight.center_of_mass
            pygame.draw.line(
                self.screen,
    ...
    def draw_info_panel(self, robot, mode_manager, grid=None, points=None, sight=None):
        ...
        # TODO: Угол до препятствий (если есть sight и центр масс) - Задание 5
        if sight is not None and sight.center_of_mass:
            cx, cy = sight.center_of_mass  # TODO: получить центр масс
            
            # TODO: Вектор от робота до центра масс
            dx = cx - robot.x  # cx - robot.x
            dy = cy - robot.y  # cy - robot.y
            
            # TODO: Угол к центру масс
            target_angle = math.atan2(dy, dx)  # math.atan2(dy, dx)
            
            # TODO: Разница углов
            angle_diff = target_angle - robot.angle  # target_angle - robot.angle
            
            # TODO: Нормализовать в диапазон [-π, π]
            # Используйте циклы while
            
            # TODO: Конвертировать в градусы
            angle_diff_deg = math.degrees(angle_diff)  # math.degrees(angle_diff)
