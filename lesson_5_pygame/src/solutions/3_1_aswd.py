...
pygame.display.set_caption("Задание 3.1: Управление WASD")
...
while running:
    ...
    # УПРАВЛЕНИЕ: проверяем нажатые клавиши
    keys = pygame.key.get_pressed()
    
    if keys[pygame.K_w]:
        robot_y += move_speed  # вверх (Y увеличивается)
    if keys[pygame.K_s]:
        robot_y -= move_speed  # вниз (Y уменьшается)
    if keys[pygame.K_a]:
        robot_x -= move_speed  # влево (X уменьшается)
    if keys[pygame.K_d]:
        robot_x += move_speed  # вправо (X увеличивается)
