...
```python
# Блок 4.3: Система энергии - реалистичные роботы! - РЕШЕНИЕ
# Добавляем батарейки, отдых и энергоменеджмент
...
class EnergyRobot:
    ...
    def can_move(self):
        """Проверяет, хватает ли энергии для движения"""
        return self.energy >= self.energy_per_step
    ...
    def move_step(self, direction):
        ...
        # Реализуем движение и трату энергии
        if direction == "up":
            self.y += 1
        elif direction == "down":
            self.y -= 1
    ...
    def rest(self):
        """Отдыхает и восстанавливает энергию"""
        old_energy = self.energy
        self.energy = min(self.max_energy, self.energy + self.energy_per_rest)
        self.rest_cycles += 1
        self.energy_history.append(self.energy)
        self.actions_log.append("rest")
...
energy_robot = EnergyRobot("Энергетик", 0, 0, "blue", 100)
...
energy_robot.execute_sequence_with_energy(test_commands)
energy_robot.show_statistics()

# ТЕСТ 2: Робот с низкой энергией
print("\n🧪 ТЕСТ 2: Робот с ограниченной энергией")

low_energy_robot = EnergyRobot("Слабак", 0, 0, "red", max_energy=50)
...
class ConservativeRobot(EnergyRobot):
    def __init__(self, name, start_x=0, start_y=0, color="blue"):
        super().__init__(name, start_x, start_y, color)
        self.low_energy_threshold = 60  # высокий порог для консерватора

# Робот-рискован (отдыхает поздно)
class RiskyRobot(EnergyRobot):
    def __init__(self, name, start_x=0, start_y=0, color="red"):
        super().__init__(name, start_x, start_y, color)
        self.low_energy_threshold = 15  # низкий порог для рисковача

conservative = ConservativeRobot("Консерватор", 0, 0, "green")
risky = RiskyRobot("Рисковач", 0, 5, "red")
...
for robot in robots:
    total_actions = robot.steps_made + robot.rest_cycles
    efficiency = robot.steps_made / max(total_actions, 1) * 100
    print(f"{robot.name}:")
    print(f"   Шагов: {robot.steps_made}, Отдыха: {robot.rest_cycles}")
    print(f"   Эффективность: {efficiency:.1f}%")
    print(f"   Финальная энергия: {robot.energy}")
...
# Дополнительный эксперимент: экстремальные условия
print(f"\n🚀 БОНУС: Экстремальные условия")

# Робот-марафонец
marathon_robot = EnergyRobot("Марафонец", 0, 0, "purple", max_energy=200)
marathon_robot.energy_per_rest = 10  # быстрое восстановление

# Сложная задача
extreme_commands = (["right"] * 10 + ["up"] * 10 + ["left"] * 10 + ["down"] * 10) * 2

print("Марафонский тест (80 команд):")
marathon_robot.execute_sequence_with_energy(extreme_commands)
marathon_robot.show_statistics()

# Итоговая статистика всех роботов
print(f"\n📈 ИТОГОВАЯ СТАТИСТИКА ЭНЕРГОСИСТЕМЫ:")
all_robots = [energy_robot, low_energy_robot, conservative, risky, marathon_robot]
...
# Анализ стратегий
print(f"\n🧠 АНАЛИЗ СТРАТЕГИЙ:")
print(f"   Консерватор: отдыхает при <{conservative.low_energy_threshold} энергии")
print(f"   Рисковач: отдыхает при <{risky.low_energy_threshold} энергии")
print(f"   Результат: {'Консерватор' if conservative.steps_made > risky.steps_made else 'Рисковач'} сделал больше шагов")
