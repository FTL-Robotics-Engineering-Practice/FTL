...
```python
# Блок 3.1: Фабрика роботов - массовое производство! - РЕШЕНИЕ
# Создаем много роботов из списка данных
...
# База данных роботов для создания концентрических квадратов
robots_data = [
    {"name": "Внешний", "x": 0, "y": 0, "color": "red"},      # старт (0,0), квадрат 4x4
    {"name": "Средний", "x": 1, "y": 1, "color": "blue"},       # старт (1,1), квадрат 3x3
    {"name": "Внутренний", "x": 2, "y": 2, "color": "green"}    # старт (2,2), квадрат 2x2
]
...
# Поиск чемпионов
most_active = max(robot_army, key=lambda r: r.steps_made)
laziest = min(robot_army, key=lambda r: r.steps_made)

print(f"   🏆 Самый активный: {most_active.name} ({most_active.steps_made} шагов)")
print(f"   😴 Самый ленивый: {laziest.name} ({laziest.steps_made} шагов)")

# Группировка по цветам
color_groups = {}
for robot in robot_army:
    if robot.color not in color_groups:
        color_groups[robot.color] = []
    color_groups[robot.color].append(robot.name)

print(f"\n🎨 РОБОТЫ ПО ЦВЕТАМ:")
for color, names in color_groups.items():
    print(f"   {color}: {', '.join(names)}")

# Дополнительный эксперимент: масштабирование
print(f"\n🚀 ЭКСПЕРИМЕНТ: Создаем еще больше роботов!")

# Автоматическое создание большой армии
big_army = []
for i in range(10):
    robot = Robot(f"Авто-{i+1}", i*3, i*2, f"color-{i%3}")
    big_army.append(robot)

print(f"   Создано дополнительно: {len(big_army)} роботов")
print(f"   Общая армия: {len(robot_army) + len(big_army)} роботов")
