...
```python
# Блок 4.2: Комбинаторика команд - математика для роботов! - РЕШЕНИЕ
# Изучаем перестановки, комбинации и случайные паттерны
...
class CombinatorialRobot:
    ...
    def test_all_permutations(self, base_commands):
        ...
        permutations = list(itertools.permutations(base_commands))
    ...
    def generate_repeated_patterns(self, single_commands, repetitions):
        ...
        patterns = []
        for command in single_commands:
            repeated_command = [command] * repetitions
    ...
    def analyze_command_frequency(self, all_experiments):
        ...
        for exp in all_experiments:
            if 'permutation' in exp:
                ...
                all_commands.extend(exp['pattern'])
        
        frequency = Counter(all_commands)
...
experimenter = CombinatorialRobot("Экспериментатор", 0, 0, "blue")

# Простой набор команд для тестирования
simple_commands = ["up", "right"]

permutation_results = experimenter.test_all_permutations(simple_commands)
...
random_walks = experimenter.create_random_walks(complex_commands, 5)
...
frequency_analysis = experimenter.analyze_command_frequency(all_experiments)
...
# Дополнительный анализ: симметрия команд
opposite_pairs = [("up", "down"), ("left", "right")]
print(f"\n🔄 АНАЛИЗ СИММЕТРИИ:")
for cmd1, cmd2 in opposite_pairs:
    count1 = frequency_analysis.get(cmd1, 0)
    count2 = frequency_analysis.get(cmd2, 0)
    balance = abs(count1 - count2)
    print(f"   {cmd1} vs {cmd2}: {count1} vs {count2} (разница: {balance})")
...
# Бонус: математический анализ
print(f"\n🧮 МАТЕМАТИЧЕСКИЙ АНАЛИЗ:")
print(f"   Факториал от {len(simple_commands)} команд: {len(permutation_results)} перестановок")
print(f"   Формула: n! где n = количество уникальных команд")

# Проверяем формулу комбинаторики
import math
n = len(set(simple_commands))
expected_permutations = math.factorial(n) if len(simple_commands) == len(set(simple_commands)) else "сложно вычислить (есть повторы)"
print(f"   Ожидаемое количество перестановок: {expected_permutations}")
