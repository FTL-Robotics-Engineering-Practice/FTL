...
class Renderer:
    ...
    def draw_info_panel(self, robot, mode_manager, brain, grid=None, points=None, sight=None):
        ...
        if sight is not None and sight.center_of_mass:  # sight is not None and sight.center_of_mass
            cx, cy = sight.center_of_mass  # получить координаты центра масс
            ...
            while angle_diff > math.pi:  # angle_diff > math.pi
            ...
            draw_text(f"{math.degrees(brain.target_angle):.1f}°")  # math.degrees(brain.target_angle)
            draw_text("ОШИБКА:", color=self.YELLOW)
            error = angle_diff_deg - math.degrees(brain.target_angle)  # angle_diff_deg - math.degrees(brain.target_angle)
