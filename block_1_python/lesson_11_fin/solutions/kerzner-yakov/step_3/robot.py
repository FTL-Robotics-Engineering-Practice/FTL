...
class Robot:
    ...
    def update(self, dt, grid=None):
        ...
        Args:
            ...
            grid: объект Grid для проверки границ (опционально)
        """
        # Обновляем позицию на основе линейной скорости и угла
        new_x = self.x + self.linear_velocity * math.cos(self.angle) * dt
        new_y = self.y + self.linear_velocity * math.sin(self.angle) * dt

        # Если передан grid, проверяем границы карты
        if grid is not None:
            robot_radius = 15  # Радиус робота в пикселях
            # Ограничиваем X (с учетом offset_x)
            min_x = grid.offset_x + robot_radius
            max_x = grid.offset_x + grid.width - robot_radius
            new_x = max(min_x, min(max_x, new_x))
            
            # Ограничиваем Y
            min_y = robot_radius
            max_y = grid.height - robot_radius
            ...
            for row in range(min_row, max_row):
                if 0 <= col < grid.cols and 0 <= row < grid.rows:
                    ...
                    if distance <= radius and distance > 0.1:
                        ...
                        # Проверяем, что клетка в секторе ±45°
    ...
    def check_collision(self, grid, desired_linear_velocity):
        ...
        # Определяем, какой сектор проверять
        if desired_linear_velocity > 0:
            # Движение вперед - проверяем передний сектор
            sector_cells = self.get_sector_cells(grid, angle_offset=0)
        else:
            # Движение назад - проверяем задний сектор
            sector_cells = self.get_sector_cells(grid, angle_offset=math.pi)
